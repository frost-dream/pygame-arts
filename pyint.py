...
pygame.display.set_caption("Pyint by Burak")
pygame.display.set_icon(pygame.image.load("imgs/icon.png"))
fillImage = pygame.transform.scale(pygame.image.load("imgs/fill.png"), (40,40))
brushImage = pygame.transform.scale(pygame.image.load("imgs/brush.png"), (25,25))
eraserImage = pygame.transform.scale(pygame.image.load("imgs/eraser.png"), (25,25))
dropperImage = pygame.transform.scale(pygame.image.load("imgs/eyedropper.png"), (30,30))
...
def draw_walls():
    wall_color = (50,50,50)
    wall_thickness = 4
    pygame.draw.rect(screen, (150,150,150), (g1.xCount * g1.cellSize, 0, sw - g1.xCount * g1.cellSize, g1.yCount*g1.cellSize))
    pygame.draw.rect(screen, (80,80,80), (0, g1.xCount * g1.cellSize, sw, sh-g1.yCount*g1.cellSize))
    pygame.draw.rect(screen, wall_color, (g1.xCount * g1.cellSize, 0, wall_thickness, g1.yCount*g1.cellSize))
    pygame.draw.rect(screen, wall_color, (0, g1.yCount*g1.cellSize-wall_thickness, sw, wall_thickness))
...
logo = pygame.transform.scale(pygame.image.load("imgs/logo.png"), (280,70))
colors1 = [[0, 0, 0], [24,24,24], [48,48,48], [64,64,64], [128,128,128],[155,155,155],[200,200,200],[255,255,255],[27,38,49],[40,55,71],[46,64,83],[52,73,94],[93,109,126],[133,146,158],[174,182,191],[214,219,223],[77,86,86],[95,106,106],[113,125,126],[149,165,166],[170,183,184],[191,201,202],[213,219,219],[229,232,232],[98,101,103],[121,125,127],[144,148,151],[189,195,199],[202,207,210],[229,231,233],[248,249,249],[255,255,255],[100,30,22],[123,36,28],[146,43,33],[192,57,43],[205,97,85],[217,136,128],[230,176,170],[242,215,213],[120,40,31],[148,49,38],[176,58,46],[220,76,60],[236,112,99],[241,148,138],[245,183,177],[250,219,216],[74,35,90],[91,44,111],[108,52,131],[142,68,173],[165,105,189],[187,143,206],[210,180,222],[232,218,239],[21,67,96],[26,82,118],[31,97,141],[41,128,185],[84,153,199],[127,179,213],[169,204,227],[212,230,241],[20,90,50],[25,111,61],[34,141,84],[34,174,96],[82,190,128],[125,206,160],[169,223,191],[212,239,223],[125,102,8],[154,125,10],[183,149,11],[230,196,15],[244,208,63],[247,220,111],[249,231,159],[252,243,207],[126,81,9],[156,100,12],[185,119,14],[242,156,18],[245,176,65],[248,196,113],[250,215,160],[253,235,208],[110,44,0],[135,54,0],[160,64,0],[211,84,0],[220,118,51],[229,152,102],[237,187,153],[246,221,204]]
colors2 = [[241,157,154],[241,179,164],[246,209,190],[252,225,213],[242,193,173],[241,175,153],[128,232,221],[124,194,246],[175,129,228],[231,132,186],[249,193,160],[183,246,175],[100,93,62],[130,123,92],[156,151,115],[86,113,80],[46,71,43],[16,42,10],[252,120,150],[193,107,188],[152,89,197],[108,66,196],[85,56,193],[30,171,215],[92,58,42],[121,84,63],[172,138,104],[200,173,139],[223,213,191],[206,159,85]]
colorCells1 = []
colorCells2 = []
...
colorScheme = 1
colorFont = pygame.font.SysFont(None, 22)
colorTexts = [colorFont.render("Skin", True, (50,50,50)), colorFont.render("Soft Hues", True, (50,50,50)),colorFont.render("Forest", True, (50, 50, 50)), colorFont.render("Sunset", True, (50,50,50)),colorFont.render("Coffee", True, (50, 50, 50))]
...
nameSurface = pygame.Surface((370,40))
nameSurface.fill(pygame.Color("White"))
fileName = "unnamed"
selectedTool = 0
...
def draw_palette(scheme):
    ...
    pygame.draw.rect(screen, (200, 200, 200), (779, 400, 170, 350))
    if scheme == 1:
    ...
    pygame.draw.rect(screen, (235, 235, 235), (positions1[-1][0] - 82, positions1[-1][1] + 27, 35, 35))
    pygame.draw.rect(screen, colorUsing, (positions1[-1][0] - 76, positions1[-1][1] + 33, 23, 23))
def paint(var):
    ...
    if sizeToDraw == 1:
        mouseRelPosX = max(penSize - 1, min(g1.xCount - 1, int(Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pygame.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 1, min(g1.yCount - 1, int(Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pygame.mouse.get_pos()[1]))))
        g1.change_color(mouseRelPosY, mouseRelPosX, colorUsing)
    if sizeToDraw == 2:
        mouseRelPosX = max(penSize - 1, min(g1.xCount - 2, int(Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pygame.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 1, min(g1.yCount - 2, int(Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pygame.mouse.get_pos()[1]))))
        g1.change_color(mouseRelPosY, mouseRelPosX, colorUsing)
        ...
        g1.change_color(mouseRelPosY + 1, mouseRelPosX, colorUsing)
    if sizeToDraw == 3:
        mouseRelPosX = max(penSize - 2, min(g1.xCount - 2, int(Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pygame.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 2, min(g1.yCount - 2, int(Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pygame.mouse.get_pos()[1]))))
        g1.change_color(mouseRelPosY, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY - 1, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX - 1, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX + 1, colorUsing)
        ...
        g1.change_color(mouseRelPosY + 1, mouseRelPosX - 1, colorUsing)
    if sizeToDraw == 4:
        mouseRelPosX = max(penSize - 2, min(g1.xCount - 3, int(Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pygame.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 2, min(g1.yCount - 3, int(Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pygame.mouse.get_pos()[1]))))
        g1.change_color(mouseRelPosY, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY - 1, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX - 1, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX + 1, colorUsing)
        ...
        g1.change_color(mouseRelPosY - 2, mouseRelPosX, colorUsing)
    if sizeToDraw == 5:
        mouseRelPosX = max(penSize - 3, min(g1.xCount - 3, int(Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pygame.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 3, min(g1.yCount - 3, int(Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pygame.mouse.get_pos()[1]))))
        g1.change_color(mouseRelPosY, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY - 1, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX - 1, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX + 1, colorUsing)
...
while True:
    ...
    for event in pygame.event.get():
            ...
            if event.button == 3:
                ...
                if pygame.mouse.get_pos()[0] < g1.xCount*g1.cellSize and pygame.mouse.get_pos()[1] < g1.yCount*g1.cellSize:
                    if selectedTool == 0 or selectedTool == 1:
                        ...
                        mouseRelPosX = max(0, min(g1.xCount - 1, int(Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pygame.mouse.get_pos()[0]))))
                        mouseRelPosY = max(0, min(g1.yCount - 1, int(Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pygame.mouse.get_pos()[1]))))
                    ...
                    if colorScheme == 1:
                        for i, Scolor in enumerate(colorCells1):
                            if Scolor.subsurface.get_rect(topleft=positions1[i]).collidepoint(pygame.mouse.get_pos()):
                                selectedColor = Scolor.color
                    elif colorScheme == 2:
                        for i, Scolor in enumerate(colorCells2):
                            if Scolor.subsurface.get_rect(topleft=positions2[i]).collidepoint(pygame.mouse.get_pos()):
                                selectedColor = Scolor.color
                    for but in S_buttons:
                        if but.subsurface.get_rect(topleft=(but.pos[0]-but.width/2, but.pos[1])).collidepoint(pygame.mouse.get_pos()):
                            but.active = True
                        else:
                            but.active = False
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if clicking:
                ...
                for but in S_buttons:
                    if but.active:
                        but.pos[0] = max(but.drawPos[0]-60, min(pygame.mouse.get_pos()[0], but.drawPos[0]+60))
                    else:
                        but.active = False
                for but in P_Buttons:
                    if but.subsurface.get_rect(topleft=but.pos).collidepoint(pygame.mouse.get_pos()):
    ...
    pygame.draw.rect(screen, (180,180,180), (779, 50, 170, 100))
    for but in B_Buttons:
        but.Draw(screen)
    for but in P_Buttons:
        but.Draw(screen)
    ...
    screen.blit(pygame.transform.scale(fillImage, (22,22)), (B_fillTool.pos[0]+3, B_fillTool.pos[1]+2))
    ...
    screen.blit(pygame.transform.scale(dropperImage, (22,22)), (B_eyeDropper.pos[0]+3, B_eyeDropper.pos[1]+2))
    ...
    if selectedTool == 0:
        ...
        if pygame.mouse.get_pos()[0] < g1.xCount * g1.cellSize and pygame.mouse.get_pos()[1] < g1.yCount * g1.cellSize:
            screen.blit(fillImage, (pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1]-35))
    elif selectedTool == 3:
        if pygame.mouse.get_pos()[0] < g1.xCount * g1.cellSize and pygame.mouse.get_pos()[1] < g1.yCount * g1.cellSize:
            screen.blit(dropperImage, (pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1]-30))
